...
class HMC(nn.Module):
    ...
    def hamiltonian_energy(self, x, p, std):
        return torch.sum(torch.square(p), dim=-1) / (2 * (std ** 2)) - self.target_log_density(x)

    def leapfrog(self, x, p, dt, L):
        # half grad step
        p = p + dt * torch.autograd.grad(torch.sum(self.target_log_density(x)), x)[0] / 2
        for i in range(L):
            # full momentum step
            x = x + dt * p
            if i != L - 1:
                p = p + dt * torch.autograd.grad(torch.sum(self.target_log_density(x)), x)[0]
        p = p + dt * torch.autograd.grad(torch.sum(self.target_log_density(x)), x)[0] / 2
        return x, p
